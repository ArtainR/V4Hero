		...
		for(const auto& folder : get_directores(modsPath))
		{
			if(std::filesystem::exists(folder + "/mod_info.json"))
			{
				...
				if (mod_file.good())
				{
					...
					try { tmp.name = mod_info["name"]; } catch(const std::exception& e) { tmp.name = "No Data"; }
					try { tmp.author = mod_info["author"]; } catch(const std::exception& e) { tmp.author = "No Data"; }
					try { tmp.version = mod_info["version"]; } catch(const std::exception& e) { tmp.version = "1.0.0"; }
